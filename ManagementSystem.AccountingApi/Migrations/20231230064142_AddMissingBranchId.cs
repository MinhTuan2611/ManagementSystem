...
{
    public partial class AddMissingBranchId : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.Sql(@"
                ...
                BEGIN
                    ...
                    FROM cte
    		                ...
    		                (
    			                ...
    			                FOR PaymentMethodCode IN (
    				                [MOMO], [CASH], [CARD], [BANKING], [POINT], [ZALO], [OTHER], [VNPAY], [SAMSUNG], [DISCOUNT])
    		                ) AS pivot_table

	                IF(SELECT COUNT(1) FROM #totalAmountByMethods) = 0
	                BEGIN
		                INSERT INTO #totalAmountByMethods ([MOMO], [CASH], [CARD], [BANKING], [POINT], [ZALO], [OTHER], [VNPAY], [SAMSUNG], [DISCOUNT])
	                ...
	                AND Description NOT LIKE '% Ket Ca%'
                    ...
                    SET @TotalRealAmountInShift = COALESCE(@totalCash, 0) 

                    -- Tinh tien thuc te con lai
	                select @totalRealReminingAmount = [CASH] + @shiftInReceiveMoney -  (@totalPayment + COALESCE(@CompanyMoneyTransferred, 0))
	                FROM #totalAmountByMethods

                    -- Tinh Tien thua thieu

	                SELECT @totalDiffAmount = @TotalRealAmountInShift - COALESCE(@totalRealReminingAmount, 0)
                    ...
                    BEGIN TRY
    	                ...
    	                BEGIN
    		                ...
    		                SELECT @shiftId,         -- ShiftId - int
    			                ...
    			                COALESCE(BANKING, 0),            -- TotalInternalConsumption - int
    			                COALESCE(MOMO, 0),            -- TotalMOMOAmount - int
    			                COALESCE(@totalPayment, 0) + COALESCE(@CompanyMoneyTransferred, 0),            -- TotalExpenses - int
    			                COALESCE(@totalPayment, 0)+ COALESCE(@CompanyMoneyTransferred, 0),            -- OtherExpense - int
    			                COALESCE(@TotalRealAmountInShift, 0),            -- ActualMoneyForNextShift - int
    			                COALESCE(@totalRealReminingAmount, 0),            -- RemindMoneyForNextShift - int
    			                IIF(@totalDiffAmount > 0, @totalDiffAmount, 0),            -- ExcessMoney - int
    			                IIF(@totalDiffAmount < 0, @totalDiffAmount, 0),            -- LackOfMoney - int
    			                SYSDATETIME(), -- ReportDate - datetime2(7)
    			                COALESCE(CARD, 0)  + COALESCE(VNPAY, 0),
				                COALESCE(POINT, 0),
				                COALESCE(ZALO, 0) + COALESCE(OTHER, 0) + COALESCE(SAMSUNG, 0) + COALESCE(DISCOUNT, 0)
    		                FROM #totalAmountByMethods
