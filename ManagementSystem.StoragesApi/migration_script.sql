...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20230908111533_update_request_table_v1')
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20230908111533_update_request_table_v1', N'6.0.20');
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20230909075910_update_request_table_v2')
BEGIN
    ALTER TABLE [Requests] ADD [Signature] nvarchar(max) NULL;
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20230909075910_update_request_table_v2')
BEGIN
    ALTER TABLE [Requests] ADD [TotalAmount] int NULL;
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20230909075910_update_request_table_v2', N'6.0.20');
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20230909113339_update_request_table_v3')
BEGIN
    ALTER TABLE [RequestItem] DROP CONSTRAINT [FK_RequestItem_Requests_RequestId];
END;
GO
...
BEGIN
    ...
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Requests]') AND [c].[name] = N'BillNumber');
    IF @var3 IS NOT NULL EXEC(N'ALTER TABLE [Requests] DROP CONSTRAINT [' + @var3 + '];');
    ALTER TABLE [Requests] ALTER COLUMN [BillNumber] nvarchar(max) NOT NULL;
END;
GO

IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20230909113339_update_request_table_v3')
BEGIN
    DROP INDEX [IX_RequestItem_RequestId] ON [RequestItem];
    DECLARE @var4 sysname;
    SELECT @var4 = [d].[name]
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[RequestItem]') AND [c].[name] = N'RequestId');
    IF @var4 IS NOT NULL EXEC(N'ALTER TABLE [RequestItem] DROP CONSTRAINT [' + @var4 + '];');
    ALTER TABLE [RequestItem] ALTER COLUMN [RequestId] int NOT NULL;
    ALTER TABLE [RequestItem] ADD DEFAULT 0 FOR [RequestId];
    CREATE INDEX [IX_RequestItem_RequestId] ON [RequestItem] ([RequestId]);
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20230909113339_update_request_table_v3')
BEGIN
    ALTER TABLE [RequestItem] ADD CONSTRAINT [FK_RequestItem_Requests_RequestId] FOREIGN KEY ([RequestId]) REFERENCES [Requests] ([RequestId]) ON DELETE CASCADE;
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20230909113339_update_request_table_v3', N'6.0.20');
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20230913160604_update_gross_profit_1')
BEGIN
    ALTER TABLE [ProductUnit] ADD [GrossProfit] real NOT NULL DEFAULT CAST(0 AS real);
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20230913160604_update_gross_profit_1', N'6.0.20');
END;
GO
